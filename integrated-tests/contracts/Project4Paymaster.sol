...
contract Project4Paymaster is Ownable {
    ...
    function validatePaymasterUserOp(
        ...
        (uint48 validUntil, uint48 validAfter, uint256 maxCostUsdcE6, uint256 usdcPerGasPrice, bytes memory sig) =
        ...
        if (!bootstrapApproval && maxCostUsdcE6 > 0) {
            postOpContext = abi.encode(userOp.sender, maxCostUsdcE6, usdcPerGasPrice);
        }
    ...
    function postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost, uint256 actualUserOpFee)
        ...
        onlyEntryPoint
    {
        // Charge USDC based on actual gas used (context: sender, maxCostUsdcE6, usdcPerGasPrice)
        if (context.length > 0 && mode == PostOpMode.opSucceeded) {
            (address sender, uint256 maxCostUsdcE6, uint256 usdcPerGasPrice) = abi.decode(context, (address, uint256, uint256));
            if (maxCostUsdcE6 > 0 && usdcPerGasPrice > 0) {
                uint256 chargeAmount = (actualGasCost * usdcPerGasPrice) / 1e6; // Convert to USDC e6 decimals
    ...
    function _decodePaymasterData(bytes calldata paymasterAndData)
        internal
        pure
        returns (uint48 validUntil, uint48 validAfter, uint256 maxCostUsdcE6, uint256 usdcPerGasPrice, bytes memory sig)
    {
        if (paymasterAndData.length < 52 + 32 * 5) revert InvalidPaymasterData();
        bytes calldata trailing = paymasterAndData[52:];
        (validUntil, validAfter, maxCostUsdcE6, usdcPerGasPrice, sig) = abi.decode(trailing, (uint48, uint48, uint256, uint256, bytes));
