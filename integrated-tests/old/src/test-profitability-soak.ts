...
const TARGET = "0xd8da6bf26964af9d7eed9e03e53415d37aa96045" as Address;
...
async function main() {
  ...
  const feeSinkEnv = (
    process.env.TOOLS_SOAK_FEE_SINK_ADDRESS ??
    process.env.TOOLS_DASHBOARD_TREASURY_ADDRESS ??
    process.env.TOOLS_PAYMASTER_CONTRACT_TREASURY_ADDRESS ??
    ""
  )
    .trim()
    .toLowerCase();
  const feeSinkUsdc = (feeSinkEnv || (paymasterAddress as string).toLowerCase()) as Address;
  ...
  if (feeSinkUsdc) {
    revenueStart = await getUSDCBalance(feeSinkUsdc);
  }
  if (paymasterAddress) {
  ...
  if (feeSinkUsdc) {
    revenueEnd = await getUSDCBalance(feeSinkUsdc);
  }
  if (paymasterAddress) {
  ...
  const revenueDelta = feeSinkUsdc ? revenueEnd - revenueStart : totalFees;
  ...
  if (feeSinkUsdc) {
    console.log("  Treasury USDC delta:", revenueDelta.toString(), "USDC (e6)");
  }
  if (paymasterAddress) {
