...
export async function deployProject4Paymaster(): Promise<Address> {
	...
	const verifier = (process.env.PAYMASTER_CONTRACT_SIGNER_ADDRESS || deployer.address) as Address;
	const treasury = (process.env.PAYMASTER_CONTRACT_TREASURY_ADDRESS || deployer.address) as Address;

	const hash = await walletClient.deployContract({
		abi: artifact.abi,
		bytecode: artifact.bytecode as `0x${string}`,
		args: [ENTRYPOINT_V07, usdc, verifier, treasury],
		gas: 5_000_000n,
	});
	...
	// No auto-deposit of ETH to EntryPoint — worker handles swap + deposit via startup/interval
