...
import { deployProject4Paymaster } from "./deploy-project4-paymaster.js";
import { deployGasBurner } from "./deploy-gas-burner.js";
...
async function applyContractDeployerConfigFromRedis(): Promise<void> {
	if (!process.env.VALKEY_URL?.trim() || !process.env.VALKEY_KEY_PREFIX?.trim()) {
		return;
	}
	const rc = await loadStrictRedisConfig("contract-deployer");
...
const main = async () => {
	await applyContractDeployerConfigFromRedis();
	if (!process.env.ANVIL_RPC?.trim()) throw new Error("ANVIL_RPC required (set in .env or Redis config:contract-deployer)");
	...
	if (process.env.CONTRACT_DEPLOYER_SKIP_DEPLOYMENTS === "true") {
		...
		if (process.env.CONTRACT_DEPLOYER_DEPLOY_GAS_BURNER === "true") {
			try {
				await deployGasBurner();
			} catch (e) {
				console.error("[GasBurner] Deploy failed:", e);
			}
		}
