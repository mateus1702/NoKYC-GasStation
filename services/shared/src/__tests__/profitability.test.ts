...
describe("computeEffectiveUnitCost", () => {
  ...
  it("applies service fee and quote buffer", () => {
    const effective = computeEffectiveUnitCost({
      baseUnitCostUsdcPerWei: base,
      serviceFeeBps: 500n,
      quoteBufferBps: 1000n,
    });
    expect(effective).to.equal(
      (base * 10_500n * 11_000n) / (10_000n * 10_000n)
    );
  });

  it("returns base when both BPS are zero", () => {
    expect(
      computeEffectiveUnitCost({
        baseUnitCostUsdcPerWei: base,
        serviceFeeBps: 0n,
  ...
  it("rounds down on fractional markup", () => {
    ...
    const upper = (small * 10_001n * 10_001n) / (10_000n * 10_000n);
