...
describe("Totals-only pricing math", () => {
  ...
  it("should compute refill deficit from min/cap (native watch address)", () => {
    const minNative = 100n * 10n ** 15n;
    const capNative = 500n * 10n ** 15n;

    const balanceLow = 50n * 10n ** 15n;
    const needNative = balanceLow < minNative ? capNative - balanceLow : 0n;
    expect(needNative).to.equal(450n * 10n ** 15n);

    const balanceAtCap = 500n * 10n ** 15n;
    const needNativeAtCap = balanceAtCap < minNative ? capNative - balanceAtCap : 0n;
    expect(needNativeAtCap).to.equal(0n);
  });
});
