...
export async function fetchRecentProcessedUserOps(limit = DEFAULT_LIMIT): Promise<UserOpsPayload> {
  ...
  try {
    ...
    for (const range of BLOCK_RANGES_TO_TRY) {
      ...
      try {
        logs = await client.getContractEvents({
          ...
          abi: [parseAbiItem("event GasCharged(address indexed sender, uint256 chargedUsdcE6, uint256 chargedWei)")],
          fromBlock,
          toBlock: block,
        });
        lastError = null;
    ...
    const mapped = logs.map((log) => {
      ...
      if (args.sender == null || args.chargedUsdcE6 == null || args.chargedWei == null) return null;
      ...
      } satisfies ProcessedUserOp;
    });
    const items: ProcessedUserOp[] = mapped.filter((x): x is ProcessedUserOp => x != null).reverse().slice(0, limit);
