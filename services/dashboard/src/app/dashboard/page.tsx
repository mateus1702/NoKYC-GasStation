...
/** Overview/hero metrics polling ÔÇö fewer RPC calls than aggressive intervals. */
              ...
              <div className="flex items-baseline gap-2 mt-1">
                ...
                {trend && (
                  <div className={`flex items-center ${trendColors[trend]}`}>
                    {trend === "up" && <span>Ôåù</span>}
                    {trend === "down" && <span>Ôåÿ</span>}
                    {trend === "neutral" && <span>ÔåÆ</span>}
                  </div>
                )}
              </div>
              {detail && (
          ...
          >
            {onRefreshMetrics && (
              <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                {isValidating && (
                  <span className="text-xs font-medium text-indigo-400">UpdatingÔÇª</span>
                )}
                <button
                  type="button"
      ...
      <div className="w-full max-w-none mx-auto px-0 xl:px-2 2xl:px-4">
        ...
        <AnimatePresence mode="wait">
              ...
              {userOpsData?.status === "error" ? (
                              ...
                              <div className="flex flex-col gap-1">
                                <div className="flex items-center gap-1">
                                  <span className={op.wasMinFeeApplied ? "text-red-400" : "text-green-400"}>
                                    {op.wasMinFeeApplied ? "ÔÜá´©Å" : "Ô£ô"} Min
                                  </span>
                                  <span className="text-xs text-slate-400">
                                ...
                                <div className="flex items-center gap-1">
                                  <span className={op.wasMaxFeeApplied ? "text-yellow-400" : "text-green-400"}>
                                    {op.wasMaxFeeApplied ? "ÔÜá´©Å" : "Ô£ô"} Max
                                  </span>
                                  <span className="text-xs text-slate-400">
                          ...
                          <td className="text-sm font-mono text-slate-300">
                            <span className="inline-flex items-center gap-2 flex-wrap">
                              <span className="truncate max-w-[100px]" title={op.transactionHash}>
                                {op.transactionHash ? `${op.transactionHash.slice(0, 8)}ÔÇª` : "-"}
                              </span>
                              {op.transactionHash && (
                                <>
                                  ...
                                  >
                                    {logsLoading === op.transactionHash ? "LoadingÔÇª" : "Logs"}
                                  </button>
                                </>
                              )}
                            </span>
                ...
                {totalUserOpsPages > 1 && (
                  <div className="flex items-center justify-between gap-4 px-4 py-3 border-t border-slate-700/50 bg-slate-800/30">
                    <p className="text-sm text-slate-400">
                      Showing {(clampedPage - 1) * USEROPS_PAGE_SIZE + 1}ÔÇô{Math.min(clampedPage * USEROPS_PAGE_SIZE, totalUserOps)} of {totalUserOps}
                    </p>
                    <div className="flex items-center gap-2">
                      <button
          ...
          <Section
            ...
            subtitle="MATIC spent (actualGasCost) per UserOp ÔÇö compare with USDC charged above"
            gridCols="grid-cols-1"
            className="xl:col-span-2 2xl:col-span-3"
          >
            <div className="overflow-hidden rounded-2xl border border-slate-700/50 bg-gradient-to-br from-slate-800/50 to-slate-700/50 backdrop-blur-sm">
              {entrypointOpsData?.status === "error" ? (
                        ...
                        >
                          ...
                          <td className="text-sm font-mono text-slate-300 truncate max-w-[140px]" title={op.userOpHash}>
                            <span className="inline-flex items-center gap-2">
                              <span className="truncate">{op.userOpHash.slice(0, 10)}ÔÇª</span>
                ...
                {totalEntryPointOpsPages > 1 && (
                  <div className="flex items-center justify-between gap-4 px-4 py-3 border-t border-slate-700/50 bg-slate-800/30">
                    <p className="text-sm text-slate-400">
                      Showing {(clampedEntryPointPage - 1) * ENTRYPOINT_OPS_PAGE_SIZE + 1}ÔÇô{Math.min(clampedEntryPointPage * ENTRYPOINT_OPS_PAGE_SIZE, totalEntryPointOps)} of {totalEntryPointOps}
                    </p>
                    <div className="flex items-center gap-2">
                      <button
